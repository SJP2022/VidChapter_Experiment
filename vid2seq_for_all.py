...
asr_pkl_root = "/share_io03_ssd/test2/chengfeng/ActivityNet_vidchapters_24_9/asr_pkl"
chapter_root = "/share_io03_ssd/test2/shijiapeng/ActivityNet_vidchapters_24_9/chapter"
...
args.model_name = os.path.join("google-t5", args.model_name)
...
model.eval()
model.to(device)
args.load = "./vid2seq_htmchapters.pth"
assert args.load
...
def chapter(vid):
    ...
    #判断文件是否存在
    json_path = os.path.join(chapter_root, vid+".json")
    if os.path.exists(json_path):
        print("%s exists." % vid)
        return
    
    video_path = os.path.join(video_root, vid+".mp4")
    asr_pkl_path = os.path.join(asr_pkl_root, vid+".pkl")
    if not os.path.exists(asr_pkl_path):
        print("asr.pkl of %s don't exist. Can't summarize this video!!!" % vid)
        return
    
    args.video_example = video_path
    args.asr_example = asr_pkl_path
    print("%s —— %s started." % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), vid)) 
    ...
    print("%s —— %s finished." % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), vid)) 
